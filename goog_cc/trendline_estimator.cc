...
TrendlineEstimator::TrendlineEstimator(
    ...
    : settings_(key_value_config),
      ...
      fp_(fopen("./trendline_estimator.txt","w+")),
      fp_raw_(fopen("./trendline_estimator_raw.txt","w+")) {
  RTC_LOG(LS_INFO)
...
void TrendlineEstimator::UpdateTrendline(double recv_delta_ms,
  ...
  fprintf(fp_,"%ld\t%f\t%f\t%f\t", arrival_time_ms, delta_ms, smoothed_delay_, trend);
...
void TrendlineEstimator::SetState(BandwidthUsage state){
  if(network_state_predictor_ ) 
        hypothesis_predicted_ = state;
  else  
        hypothesis_ = state;
}
...
void TrendlineEstimator::Detect(double trend, double ts_delta, int64_t now_ms) {
  ...
  fprintf(fp_,"%f\t%f\t", modified_trend, threshold_);
...
void TrendlineEstimator::UpdateThreshold(double modified_trend,
  ...
  //threshold_ = rtc::SafeClamp(threshold_, 9.f, 600.f);
  last_update_ms_ = now_ms;
}
