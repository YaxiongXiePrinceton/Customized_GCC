...
void LinkCapacityTracker::OnRateUpdate(absl::optional<DataRate> acknowledged,
 ...
 if (acknowledged_target.bps() > capacity_estimate_bps_) {
  ...
  //printf("LinkCapacityTracker::OnRateUpdate -> ack rate:%ld target:%ld ack_target:%ld capacity:%f\n", 
  //      acknowledged->kbps(), target.kbps(), acknowledged_target.kbps(), capacity_estimate_bps_);
...
void SendSideBandwidthEstimation::SetSendBitrate(DataRate bitrate,
  ...
  delay_based_limit_ = DataRate::PlusInfinity();
  //printf("SendSideBandwidthEstimation::SetSendBitrate call -> UpdateTargetBitrate");
...
DataRate SendSideBandwidthEstimation::target_rate() const {
  ...
  if (receiver_limit_caps_only_){
    //printf("Receiver limted:%ld\n", receiver_limit_.kbps()); 
...
void SendSideBandwidthEstimation::UpdateEstimate(Timestamp at_time) {
  ...
  if (last_fraction_loss_ == 0 && IsInStartPhase(at_time)) {
    ...
    if (new_bitrate != current_target_) {
      ...
      //printf("1-> SendSideBandwidthEstimation::UpdateEstimate:%ld\n", new_bitrate.kbps());
      UpdateTargetBitrate(new_bitrate, at_time);
      return;
    }
  }
  ...
  if (loss_based_bandwidth_estimation_.InUse()) {
    ...
    //printf("2-> SendSideBandwidthEstimation::UpdateEstimate:%ld\n", new_bitrate.kbps());
    UpdateTargetBitrate(new_bitrate, at_time);
    return;
  }
  ...
  if (time_since_loss_packet_report < 1.2 * kMaxRtcpFeedbackInterval) {
    ...
    if (current_target_ < bitrate_threshold_ || loss <= low_loss_threshold_) {
      ...
      //printf("3-> SendSideBandwidthEstimation::UpdateEstimate:%ld bitrate_th:%ld\n", 
                //new_bitrate.kbps(), bitrate_threshold_.kbps());
      UpdateTargetBitrate(new_bitrate, at_time);
      return;
      ...
      if (loss <= high_loss_threshold_) {
        ...
        if (!has_decreased_since_last_fraction_loss_ &&
          ...
          //printf("4-> SendSideBandwidthEstimation::UpdateEstimate:%ld\n", new_bitrate.kbps());
          UpdateTargetBitrate(new_bitrate, at_time);
          return;
        }
      }
...
void SendSideBandwidthEstimation::UpdateEstimate_Yx(Timestamp at_time, uint32_t lte_capacity) {
  if (rtt_backoff_.CorrectedRtt(at_time) > rtt_backoff_.rtt_limit_) {
    if (at_time - time_last_decrease_ >= rtt_backoff_.drop_interval_ &&
        current_target_ > rtt_backoff_.bandwidth_floor_) {
      time_last_decrease_ = at_time;
  ...
  if (time_since_loss_packet_report < 1.2 * kMaxRtcpFeedbackInterval) {
    ...
    if (current_target_ < bitrate_threshold_ || loss <= low_loss_threshold_) {
      ...
      DataRate new_bitrate = DataRate::BitsPerSec( lte_capacity);
...
DataRate SendSideBandwidthEstimation::GetUpperLimit() const {
  ...
  //printf("SendSideBandwidthEstimation::GetUpperLimit -> delay_based_limit_:%ld receiver_limit_:%ld max_bitrate_configured:%ld\n",
  //          delay_based_limit_.kbps(), receiver_limit_.kbps(), max_bitrate_configured_.kbps());
  return upper_limit;
}
...
void SendSideBandwidthEstimation::UpdateTargetBitrate(DataRate new_bitrate,
                                                      Timestamp at_time) {
  //DataRate uplimit = GetUpperLimit();
  new_bitrate = std::min(new_bitrate, GetUpperLimit());
  //printf("SendSideBandwidthEstimation::UpdateTargetBitrate -> UpperLimit:%ld new_bitrate:%ld ", 
  //          uplimit.kbps(), new_bitrate.kbps());
  //printf("New bitrate:%ld\n", new_bitrate.kbps());
  if (new_bitrate < min_bitrate_configured_) {
    MaybeLogLowBitrateWarning(new_bitrate, at_time);
    //printf(" smaller than min_bitrate_configured_");
    new_bitrate = min_bitrate_configured_;
  }
  current_target_ = new_bitrate;
  //printf("final target:%ld\n", current_target_.kbps());
