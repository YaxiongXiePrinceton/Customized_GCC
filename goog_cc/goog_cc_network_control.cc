...
int64_t timeSinceEpochMillisec() {
  using namespace std::chrono;
  return duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
}


uint32_t queryLTEScopeCapacity(int ltescope_sock_){
    // queury LTEScope the capacity
    char buf[100];
    buf[0] = 0xAA;
    buf[1] = 0xAA;
    buf[2] = 0xAA;
    ...
    uint32_t capacity = 0;
    uint32_t capacity_hm = 0;
    int recvLen;

    uint64_t WAIT_TIME_MS = 1000;

    if(ltescope_sock_ > 0){
      send(ltescope_sock_, buf, 4, 0);
      // wait for the answer
      uint64_t start_time = timeSinceEpochMillisec();
      while(true){
        recvLen = recv(ltescope_sock_, recvBuf, 100, 0);
        if( recvLen > 0){
          if( recvBuf[0] == (char)0xBB && recvBuf[1] == (char)0xBB &&
              recvBuf[2] == (char)0xBB && recvBuf[3] == (char)0xBB ){
            uint32_t tmp         = 0;

            memcpy(&tmp, recvBuf+4, 4);
            capacity    = ntohl(tmp);
            memcpy(&tmp, recvBuf+8, 4);
            capacity_hm = ntohl(tmp);

            printf("Capacity is:%d capacity hm is:%d \n",capacity, capacity_hm);
            break;
          }
        }
        uint64_t curr_time = timeSinceEpochMillisec();
        if(curr_time - start_time > WAIT_TIME_MS){
          break;
        }
      }
    }else{
      printf("LTEScope is not set!\n");
    }
    return capacity;
}
...
GoogCcNetworkController::GoogCcNetworkController(NetworkControllerConfig config,
    ...
    : key_value_config_(config.key_value_config ? config.key_value_config
      ...
      fp_(fopen("./google_cc_network_control.txt","w+")) {
...
NetworkControlUpdate GoogCcNetworkController::OnProcessInterval(
  ...
  // Update the capacity using LTE or not?
  // Yaxiong: qurey the capacity
  uint32_t capacity = 0;
  if(ltescope_sock_ > 0){
    capacity = queryLTEScopeCapacity(ltescope_sock_);
    printf("\n queried capacity:%d \n", capacity);
    //bandwidth_estimation_->UpdateEstimate_Yx(msg.at_time, 0.95 * capacity);
    bandwidth_estimation_->UpdateEstimate_Yx(msg.at_time, capacity);
  }else{
...
NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
  ...
  if(ltescope_sock_ > 0){
    capacity = queryLTEScopeCapacity(ltescope_sock_);
    printf("\n queried capacity:%d \n", capacity);
    if( result.bwUsage == BandwidthUsage::kBwNormal || 
            result.bwUsage == BandwidthUsage::kBwUnderusing){
        //KilobitsPerSec;
        //result.target_bitrate = DataRate::BitsPerSec(0.95 * capacity);
        result.target_bitrate = DataRate::BitsPerSec(capacity);
     }else{
        printf("overuse!\n");
        //result.target_bitrate = DataRate::BitsPerSec(10000000);
     }
  }

  uint64_t curr_time = timeSinceEpochMillisec();
  fprintf(fp_, "%ld\t%d\t", curr_time, capacity);
  // state kBwNormal kBwUnderusing kBwOverusing

  switch (result.bwUsage){
    case BandwidthUsage::kBwNormal:
        fprintf(fp_, "%d\n", 0);
        //printf("GoogCcNetworkController::OnTransportPacketsFeedback-> State: kBwNormal! \n");
        break;
    case BandwidthUsage::kBwUnderusing:
        fprintf(fp_, "%d\n", 1);
        //printf("GoogCcNetworkController::OnTransportPacketsFeedback-> State: kBwUnderusing! \n");
        break;
    case BandwidthUsage::kBwOverusing:
        fprintf(fp_, "%d\n", 2);
        //printf("GoogCcNetworkController::OnTransportPacketsFeedback-> State: kBwOverusing! \n");
        break;
    default:
        fprintf(fp_, "%d\n", 111);
        //printf("GoogCcNetworkController::OnTransportPacketsFeedback-> state not supported! \n");
  }

  if (result.updated) {
    //printf("Before Maybe rate:%ld \n", result.target_bitrate.bps());
    if (result.probe) {
      //printf("bandwidth_estimation->SetSendBitrate\n");
    ...
    //printf("After Maybe rate:%ld \n", result.target_bitrate.bps());
    if (update.target_rate) { 
        //printf("After Maybe target transfer rate: %ld\n", update.target_rate->target_rate.kbps());
    }
  }
...
void GoogCcNetworkController::MaybeTriggerOnNetworkChanged(
  ...
  if ((loss_based_target_rate != last_loss_based_target_rate_) ||
    ...
    //printf("target push back rate:%ld\n", target_rate_msg.target_rate.kbps());
