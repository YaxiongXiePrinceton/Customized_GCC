...
DelayBasedBwe::DelayBasedBwe(const WebRtcKeyValueConfig* key_value_config,
    ...
    : event_log_(event_log),
      ...
      alr_limited_backoff_enabled_(absl::StartsWith(
          ...
          "Enabled")),
      fp_(fopen("./delay_based_bwe.txt","w+")),
      delay_window_ms_(1000){
  RTC_LOG(LS_INFO)
    ...
    curr_time_ms = (uint32_t) ((time_now.tv_sec * 1000) + (time_now.tv_usec / 1000));

    minmax_reset(&win_delay_, curr_time_ms, 100000000);
}
...
DelayBasedBwe::Result DelayBasedBwe::IncomingPacketFeedbackVector(
  ...
  //printf("There exists %d report inside the report vector\n", cnt);
...
void DelayBasedBwe::IncomingPacketFeedback(const PacketResult& packet_feedback,
  ...
  if (use_new_inter_arrival_delta_) {
    ...
    //Yaxiong: 
    uint32_t DELAY_THRD = 40;
    uint32_t oneway_32_ms; 
    uint64_t oneway_64_ms;
    oneway_64_ms = (uint64_t)packet_feedback.receive_time.ms() - 
                        (uint64_t)packet_feedback.sent_packet.send_time.ms();
    oneway_32_ms = (uint32_t) oneway_64_ms;
    // windowed minmax update
    minmax_running_min(&win_delay_, delay_window_ms_, (uint32_t) at_time.ms(), oneway_32_ms);
    // get the minimum delay
    uint32_t min_oneway_ms = minmax_get(&win_delay_);
    //oneway_32 = timestamp - (uint32_t)packet_feedback.receive_time.ms();
    uint32_t delay_delta = oneway_32_ms - min_oneway_ms;

    //printf("onway_delay 64: %ld 32: %d min:%d\n", oneway_64_ms, oneway_32_ms, min_oneway_ms);
    ...
    fprintf(fp_, "%ld\t%ld\n",(uint64_t)packet_feedback.sent_packet.send_time.ms(),
                            (uint64_t)packet_feedback.receive_time.ms());
    ...
    if( (delay_detector_for_packet->State() == BandwidthUsage::kBwOverusing) && 
            delay_delta < DELAY_THRD){
        delay_detector_for_packet->SetState(BandwidthUsage::kBwNormal);
    }
    if(delay_detector_for_packet->State() == BandwidthUsage::kBwOverusing){
        printf("OverUSE onway: %d min:%d\n", oneway_32_ms, min_oneway_ms);
    }
  }
}
...
DelayBasedBwe::Result DelayBasedBwe::MaybeUpdateEstimate(
  ...
  result.bwUsage = active_delay_detector_->State(); 
